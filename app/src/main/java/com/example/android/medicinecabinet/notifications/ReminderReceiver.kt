...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (ContextCompat.checkSelfPermission(
            ...
            val interval = intent.getIntExtra(Constance.KEY_INTERVAL, -1).takeIf { it != -1 }

            val selectedDaysInt = intent.getIntArrayExtra(Constance.KEY_SELECTED_DAYS)
            val selectedDays = selectedDaysInt?.map { WeekDay.fromCalendar(it) }
            ...
            if (medId != -1 && timeId != -1 && timeValue.isNotEmpty()) {
                ...
                Alarm.singleScheduleAlarm(context, medId, medName, nextTime, interval, selectedDays)
            }
